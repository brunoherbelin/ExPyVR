...
class ModuleMain(DrawableModule):
    ...
    def __init__(self, controller, initConfig=None, runConfigs=None):
        ...
        line = ['Routine', 'exp_time', 'display_time']
    ...
    def update(self, dt):
        if self.started:
            self.counter += 1
            # Log time
            name = "UPDATE" + str(self.counter)
            line = [name, str(self.controller.gTimeManager.experimentTime()), str(self.controller.gTimeManager.displayTime())]
            # Log info
            self.csvLogger.writerow(line)
